...
@section('header')
    Gestión de Operadores <span class="text-cyan-600 text-lg font-medium ml-2">| Supervisor</span>
@endsection
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div class="flex gap-2">
        <a href="{{ route('supervisor.operadores.create') }}" class="inline-flex items-center px-4 py-2 bg-cyan-600 border border-transparent rounded-lg font-bold text-xs text-white uppercase tracking-widest hover:bg-cyan-700 focus:bg-cyan-700 active:bg-cyan-900 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-md shadow-cyan-500/30">
            <i class="fas fa-user-plus mr-2"></i> Nuevo Operador
        </a>
        <a href="{{ route('supervisor.operadores.deleted') }}" class="inline-flex items-center px-4 py-2 bg-white border border-slate-300 rounded-lg font-bold text-xs text-slate-700 uppercase tracking-widest shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
            <i class="fas fa-trash-alt mr-2 text-rose-500"></i> Eliminados
        </a>
    </div>
    
    <div class="relative w-full sm:w-72">
        <input type="text" id="searchInput" placeholder="Buscar operador..." class="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-all shadow-sm">
        ...
        <table class="w-full text-left border-collapse" id="usersTable">
            <thead>
                <tr class="bg-slate-50 text-slate-600 text-xs uppercase tracking-wider">
                    ...
                    <th class="p-4 font-semibold border-b border-slate-100">Turno</th>
                ...
                <tr class="hover:bg-slate-50 transition-colors group">
                    ...
                    <td class="p-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('supervisor.operadores.edit', $empleado->idEmpleado) }}" class="text-slate-400 hover:text-cyan-600 transition-colors" title="Editar">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('supervisor.operadores.destroy', $empleado->idEmpleado) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
